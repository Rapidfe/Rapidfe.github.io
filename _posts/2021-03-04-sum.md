...
"L에(또는 R에) 1을 더하고(빼고) 상위 레벨로 올라갈 때, 입력받은 범위보다 더 오른쪽까지(왼쪽까지) 포함하는 노드가 나오진 않을까?" 와 같은 의문을 처음에 가졌지만 시뮬레이션을 해보면 L과 R이 동시에 구간을 좁히며 탐색하기 때문에 그런 일이 발생하기 전에 반복문이 종료된다.
